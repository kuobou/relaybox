...
```bash
curl -fsSL https://raw.githubusercontent.com/kuobou/kuobox/main/install.sh -o install.sh && bash install.sh
```
...
```bash
cd /opt/kuobox && git pull && systemctl restart kuobox && echo "更新完成"
```
...
cd /opt/kuobox
git init
git remote add origin https://github.com/kuobou/kuobox.git
...
```bash
# 面板
systemctl restart kuobox     # 重啟面板
systemctl stop kuobox        # 停止面板
journalctl -u kuobox -f      # 查看面板日誌
...
1. **修改預設密碼**：安裝時設定強密碼，或編輯 `/opt/kuobox/.env` 後重啟
